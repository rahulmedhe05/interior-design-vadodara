  ...
  if (area) {
    return {
      ...
      openGraph: {
        ...
        description: `Expert interior design services in ${area.name}, Vadodara`,
        url: pageUrl,
        type: "website",
        images: [{ url: `${siteUrl}/og-image.jpg`, width: 1200, height: 630, alt: `${area.name} Interior Design` }],
      },
      twitter: {
        card: "summary_large_image",
        title: `${area.name} Interior Design in Vadodara`,
...
export function KeywordPageRenderer({ slug, siteUrl }: PageProps) {
  const keywordPage = getKeywordPageContent(slug);
  if (!keywordPage) return null;
...
export function AreaPageRenderer({ slug, siteUrl }: PageProps) {
  const area = getAreaContent(slug);
  if (!area) return null;
  ...
  const breadcrumbs = [
    { label: 'Home', url: siteUrl },
    { label: 'Service Areas', url: `${siteUrl}/areas` },
